package com.sitaluo.tableviewDemo;
import com.sitaluo.tableviewDemo.data.Data;
...
public class TableViewWithButtonExample extends Application {
  ...
  @Override public void start(final Stage stage) {
    ...
    // create a table.
    final TableView<TableViewPerson> table = new TableView<>(
      FXCollections.observableArrayList(
       Data.getTableViewPersonList()
      )
    );
 
    // define the table columns.
    TableColumn<TableViewPerson, Integer> idCol = new TableColumn<>("ID");
    idCol.setCellValueFactory(new PropertyValueFactory("id"));
    TableColumn<TableViewPerson, String> firstNameCol = new TableColumn<>("First Name");
    firstNameCol.setCellValueFactory(new PropertyValueFactory("firstName"));
    TableColumn<TableViewPerson, String> lastNameCol = new TableColumn<>("Last Name");
    lastNameCol.setCellValueFactory(new PropertyValueFactory("lastName"));
    TableColumn<TableViewPerson, Boolean> actionCol = new TableColumn<>("Action");
    actionCol.setSortable(false);
 
    // define a simple boolean cell value for the action column so that the column will only be shown for non-empty rows.
    actionCol.setCellValueFactory(new Callback<TableColumn.CellDataFeatures<TableViewPerson, Boolean>, ObservableValue<Boolean>>() {
      @Override public ObservableValue<Boolean> call(TableColumn.CellDataFeatures<TableViewPerson, Boolean> features) {
        return new SimpleBooleanProperty(features.getValue() != null);
      }
    });
 
    // create a cell value factory with an add button for each row in the table.
    actionCol.setCellFactory(new Callback<TableColumn<TableViewPerson, Boolean>, TableCell<TableViewPerson, Boolean>>() {
      @Override public TableCell<TableViewPerson, Boolean> call(TableColumn<TableViewPerson, Boolean> personBooleanTableColumn) {
    	  return new AddPersonCell(stage, table);
      }
    });
 
    table.getColumns().setAll(idCol,firstNameCol, lastNameCol, actionCol);
  ...
  private class AddPersonCell extends TableCell<TableViewPerson, Boolean> {
    ...
    HBox hbox = new HBox();
    Image image = new Image("http://icons.iconarchive.com/icons/icons-land/vista-people/72/Historical-Viking-Female-icon.png",20,20,true,true);
    private ImageView headImgView = new ImageView(image);
    /**
    ...
    AddPersonCell(final Stage stage, final TableView table) {
      ...
      hbox.getChildren().add(headImgView);
      ...
      addButton.setOnAction(new EventHandler<ActionEvent>() {
        @Override public void handle(ActionEvent actionEvent) {
          ...
          TableViewPerson person = (TableViewPerson) table.getSelectionModel().getSelectedItem();
      ...
      editButton.setOnAction(e ->{
    	  table.getSelectionModel().select(getTableRow().getIndex());
    	  TableViewPerson person = (TableViewPerson) table.getSelectionModel().getSelectedItem();
          System.out.println("edit->"+person.getFirstName());
      });
    }
   ...
   */
  private void showAddPersonDialog(Stage parent, final TableView<TableViewPerson> table, double y) {
    ...
    ok.setOnAction(new EventHandler<ActionEvent>() {
      @Override public void handle(ActionEvent actionEvent) {
        ...
        table.getItems().add(nextIndex, new TableViewPerson(12345,firstNameField.getText(), lastNameField.getText(),""));
